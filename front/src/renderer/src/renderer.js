...
import './main.css'
import { apiFetch, mostrarErrorApi } from './shared/api.js'
import { initTvModule } from './modules/tv.js'
import { initPlayerModule, Player, formatTime } from './modules/player.js'
...
function mostrarHero(item) {
  ...
  document.getElementById('hero-img').src = item.poster || ''
...
async function inicializarApp() {
  ...
  try {
    bibliotecaLocal = await window.api.getLibrary()
...
async function cargarExtensiones() {
  try {
    ...
    // Guardamos la lista para mostrarla como accesos dentro del Inicio
    // (ya no se dibuja en la barra lateral).
  ...
  try {
    ...
    if (tieneContinuar) {
      gridCatalogo.appendChild(
        crearFila({ titulo: '▶ Continuar viendo', tipo: 'historial', items: hist.slice(0, 20) })
      )
    }
...
function crearTarjetaHTML(item, forceProvider = null) {
  ...
  card.innerHTML = `
    <div class="media-card__img-wrap aspect-[2/3]">
      <img src="${item.poster}" alt="${item.titulo}" loading="lazy" class="media-card__img">
  ...
  btnFav.onclick = async (e) => {
    e.stopPropagation()
    bibliotecaLocal.favoritos = await window.api.toggleFavorite(item)
...
function ejecutarReproduccionEpisodio(ep, index, tituloSerie) {
  ...
  if (progreso && progreso.visto) {
    ...
    document.getElementById('btn-reanudar-si').onclick = () => {
      ...
      forzarReinicio = true
      Player.procesarReproduccion(ep.url, tituloVideo, index)
    }
...
async function abrirDetalles(urlPath, providerId = extensionActual) {
  ...
  try {
    ...
    detallePoster.src = detalles.poster
    if (detalleHeroBg) {
    ...
    const crearBotonEpisodio = (ep, index) => {
      ...
      btnEp.innerHTML = `
        ...
        <div class="ep-row__thumb">
          <img src="${posterEp}" alt="" loading="lazy">
...
btnDetalleFav.addEventListener('click', async () => {
  if (objetoAnimeActual) {
    bibliotecaLocal.favoritos = await window.api.toggleFavorite(objetoAnimeActual)
    actualizarEstrellasVisuales()
...
btnDetalleReset.addEventListener('click', async () => {
  ...
  const resultado = await window.api.clearProgress(objetoAnimeActual, urls)
...
window.addEventListener('keydown', (e) => {
  ...
  if (e.target && e.target.tagName === 'INPUT' && !['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'Escape', 'Enter'].includes(e.key)) {
