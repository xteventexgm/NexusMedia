...
export function initTvModule() {
  ...
  function ponerLogoCanal(img, poster) {
    img.src = poster || PLACEHOLDER_LOGO
    img.onerror = () => {
      img.onerror = null
...
`
  ...
  async function toggleFavTv(c) {
    bibliotecaLocal.favoritos = await window.api.toggleFavorite(canalAObjetoFav(c))
  }
  ...
  function reproducirStreamTv(url) {
    if (tvHls) {
      tvHls.destroy()
      tvHls = null
    }
    ...
    const onOk = () => tvPlayerMsg.classList.add('hidden')
    const onErr = () => {
    ...
    if (/\.m3u8/i.test(url) && Hls.isSupported()) {
      tvHls = new Hls({ enableWorker: true, lowLatencyMode: true })
      tvHls.loadSource(url)
      tvHls.attachMedia(tvVideo)
      tvHls.on(Hls.Events.MANIFEST_PARSED, () => {
        tvVideo.play().then(onOk).catch(onOk)
      })
      tvHls.on(Hls.Events.ERROR, (_e, data) => {
        if (data.fatal) onErr()
      })
    } else {
      tvVideo.src = url
      tvVideo.play().then(onOk).catch(onErr)
      tvVideo.onerror = onErr
    }
  }

  function cerrarTvPlayer() {
    if (tvHls) {
      tvHls.destroy()
      tvHls = null
    }
