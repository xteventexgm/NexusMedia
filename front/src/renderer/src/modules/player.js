...
export function initPlayerModule(getCtx) {
  ...
  function mostrarControlesReproductor(autoOcultar = true) {
    if (iframePlayer && !iframePlayer.classList.contains('hidden')) return
  ...
  function reproducirServidor(url) {
    ...
    if (esVideoNativo) {
      ...
      if (urlLimpia.includes('m3u8') && Hls.isSupported()) {
        ...
        ctx.hls.attachMedia(videoPlayer)
        ctx.hls.on(Hls.Events.MANIFEST_PARSED, () => {
          videoPlayer.play()
          mostrarControlesReproductor()
        })
        ctx.hls.on(Hls.Events.ERROR, (_e, data) => {
          if (data.fatal) {
      ...
      topBarReproductor.classList.remove('opacity-0', 'pointer-events-none')
      mensajeReproductor.textContent =
        'Reproducción embebida — usa el mouse en la página del servidor.'
      mensajeReproductor.classList.remove('hidden')
      clearTimeout(timeoutControles)
      timeoutControles = setTimeout(() => mensajeReproductor.classList.add('hidden'), 5000)
    }
  }
  ...
  Player.procesarReproduccion = async function procesarReproduccion(
    ...
    try {
      ...
      reproducirServidor(ctx.servidoresActuales[0].url)
      selectorServidor.onchange = (e) =>
  ...
  btnCerrarReproductor.addEventListener('click', () => {
    ...
    guardarProgresoEpisodioActual().finally(() => {
      videoPlayer.pause()
      videoPlayer.src = ''
      iframePlayer.src = ''
      if (ctx.hls) ctx.hls.destroy()
